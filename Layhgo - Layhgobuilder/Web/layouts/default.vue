...
export default {
	...
	created() {
		...
		firebase.auth().onAuthStateChanged( user => {
			if (user) {
				...
				if (this.$route.query.show === "fieldlab_license")
					this.$bus.$emit('onOpenFieldlabLicense');
				else if (this.$route.query.show == "enterprise_license")
					this.$bus.$emit('onOpenEnterpriseLicense');
			}
			else if (this.$route.query.show === "fieldlab_license" || this.$route.query.show == "enterprise_license") {
	...
	mounted() {
		...
		this.$bus.$on('onOpenFieldlabLicense', () => {
			this.$refs.auth.open(3);
			this.$refs.auth.setProduct('Fieldlab');
		})

		this.$bus.$on('onOpenEnterpriseLicense', () => {
			this.$refs.auth.open(3);
			this.$refs.auth.setProduct('Enterprise');
		})

		this.$bus.$on('onOpenChangelog', blog => {
			this.$refs.changelog.open(blog);
		})
